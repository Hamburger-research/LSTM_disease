...
WORD_LENTH = 5572
EMBEDDING_DIM = 100     # word embedding will be 2 dimension for 2d visualization
CLASS_NUM = 2
UNITS_NUM = 50
PROJECTION_NUM = 64
...
def lstm_model(embed):
   ...
   # lstm_cell_3 = tf.nn.rnn_cell.BasicLSTMCell(UNITS_NUM)
    ...
    lstm_cell_1 = tf.nn.rnn_cell.DropoutWrapper(lstm_cell_1, output_keep_prob= 0.5)
   ...
   # stacked_lstm = tf.contrib.rnn.MultiRNNCell(cells=[lstm_cell_1,lstm_cell_2,lstm_cell_3])
    ...
    prediction_prob = tf.argmax(prediction, 1)  
    ...
    #loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits = prediction,labels = target))   #交叉熵损失函数
    ...
    return BANTCH_SIZE, optimizer, loss, accuracy, input_data, target, prediction, all_outputs, softmax_w, softmax_b, embedding, prediction_prob, dropout_keep_prob
